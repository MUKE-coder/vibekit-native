...
export async function GET(request: Request) {
  ...
  const data = await prisma.post.findMany({ where: { userId: session.user.id } });
...
export async function GET(request: Request) {
  ...
  const limit = Math.min(Number(url.searchParams.get('limit') ?? PAGE_LIMIT), 50);
...
export function CreatePostForm({ onSubmit }: { onSubmit: (v: CreatePost) => Promise<void> }) {
  const [submitting, setSubmitting] = React.useState(false);
  const { control, handleSubmit, formState: { errors } } = useForm<CreatePost>({
    resolver: zodResolver(CreatePostSchema),
  ...
  return (
    <View className="gap-4">
      ...
      />
      {/* ... */}
      <Button title="Save" loading={submitting} onPress={handleSubmit(submit)} />
    </View>
  );
}
```
...
export async function GET(request: Request, { reference }: { reference: string }) {
  const session = await auth.api.getSession({ headers: request.headers });
  if (!session) return Response.json({ error: 'Unauthorized' }, { status: 401 });
...
<StripeProvider publishableKey={process.env.EXPO_PUBLIC_STRIPE_PUBLISHABLE_KEY!} merchantIdentifier="merchant.com.yourapp">
  {children}
</StripeProvider>
```
